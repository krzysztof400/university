...
generator_options = {
    'r': './random_generator',
    'd': './descending_generator',
    'a': './ascending_generator'
}

sorts = ["./insertion_sort", "./DP_quick_sort", "./merge_sort", "./quick_sort", "./tim_sort"]
...
for i in range(1, 10):  # Loop through 9 different input sizes
    n = i * gap
    for generator in generators:
        for sort in sorts:
            # Skip insertion sort for large inputs to avoid long runtime
            if n > 999 and sort == "./insertion_sort":
                continue
...
colors = {'./insertion_sort': 'blue', './DP_quick_sort': 'red', 
          './merge_sort': 'green', './quick_sort': 'orange', './tim_sort': 'purple'}
markers = {'./random_generator': 'o', './descending_generator': 's', './ascending_generator': '^'}
linestyles = {'./random_generator': '-', './descending_generator': '--', './ascending_generator': '-.'}
...
for sort in sorts:
    for generator in generators:
        ...
        sort_name = sort.split('_sort')[0].split('/')[-1]
        gen_name = generator.split('_generator')[0].split('/')[-1]
        label = f"{sort_name} - {gen_name}"
