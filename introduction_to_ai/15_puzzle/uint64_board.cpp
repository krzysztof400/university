...
public:
    ...
    PuzzleState generateRandomSolvableState(int maxDistance = 0) const {
        ...
        if (maxDistance == 0) {
            // Generate a random solvable state by shuffling
            std::vector<int> values(size * size);
            std::iota(values.begin(), values.end() - 1, 1);
            values.back() = 0;
            ...
            do {
                ...
                PuzzleState state = PuzzleState::fromVector(values, size);
            ...
            // Generate a state by making random moves from solved state
...
public:
    // Run experiments comparing different heuristics
    void runExperiments(int num_trials = 10, int max_distance = 0, bool only_manhattan = false) {
        ...
        for (int i = 0; i < num_trials; ++i) {
            std::cout << "Trial " << (i + 1) << "/" << num_trials << std::endl;
        ...
        std::cout << "Heuristic: Misplaced Tiles" << std::endl;
        std::cout << "- Average steps: " << mean(misplaced_steps) << std::endl;
        std::cout << "- Average visited states: " << mean(misplaced_visited) << std::endl;
        std::cout << "- Average solution time: " << mean_double(misplaced_time) << " s\n" << std::endl;
        
        std::cout << "Heuristic: Manhattan Distance" << std::endl;
        std::cout << "- Average steps: " << mean(manhattan_steps) << std::endl;
        std::cout << "- Average visited states: " << mean(manhattan_visited) << std::endl;
        std::cout << "- Average solution time: " << mean_double(manhattan_time) << " s" << std::endl;
    }
};
...
int main(int argc, char* argv[]) {
    ...
    if (argc > 1) {
        try {
            ...
            if (argc > 2) {
                only_manhattan = (std::stoi(argv[2]) == 1);
            }
            if (argc > 3) {
                tries = std::stoi(argv[3]);
            }
    ...
    // Run experiments
    std::cout << "\nDo you want to run full experiments? [Y/n]" << std::endl;
    char choice;
    std::cin >> choice;
    if (choice != 'n' && choice != 'N') {
        std::cout << "\nRunning experiments (this may take a few minutes)..." << std::endl;
        puzzle.runExperiments(tries, max_distance, only_manhattan);
    }
