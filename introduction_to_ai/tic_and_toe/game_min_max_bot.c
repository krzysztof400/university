...
int evaluate(int player) {
    ...
    for (int i = 0; i < 48; i++) {
        ...
        if (playerCount == 3 && !isPartOfFour) score -= 500;
        
        // If opponent has three in a row, it's a win for player
        if (opponentCount == 3 && !isPartOfFour) score += 500;
    }
...
/**
 * Check if player has three in a row (losing condition)
 */
bool hasThreeInARow(int player) {
    for (int i = 0; i < 48; i++) {
        ...
        // Check if this three is part of a potential four (not a losing condition)
        if (hasThree) {
            bool isPartOfFour = false;
            
            // This is simplified - ideally we'd check all four-in-a-row patterns
            // that could contain this three-in-a-row pattern
            
            // For now, we'll say it's a losing condition
            if (!isPartOfFour) return true;
        }
    }
    return false;
}
...
int findBestMove(int player) {
    ...
    for (int i = 0; i < SIZE; i++) {
        for (int j = 0; j < SIZE; j++) {
            if (board[i][j] == 0) {
                board[i][j] = player;
                int moveVal = minimax(3, ALPHA, BETA, player, 0);
