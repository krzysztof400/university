...
def fit_with_one_nureon_in_layer_with_activation_neural_net(input, label, N=10000, alpha = 0.0001):
    w1 = np.random.normal(size=(2,), loc=0.1, scale=0.1) # initial values of weights
    w2 = np.random.normal(size=(2,), loc=0.1, scale=0.1)
    b1 = np.random.normal(size=(1,), loc=0.1, scale=0.1)
    b2 = np.random.normal(size=(1,), loc=0.1, scale=0.1)
...
def fit_with_n_nureon_in_layer_neural_net(data, labels, n_neuron=10, N=10000, alpha = 0.0001, batch_size=3):
    ...
    for i in range(N):
        j = np.random.randint(0, len(data), batch_size)
        input = data[j]
        label = labels[j]
...
for name, ds in dataset.items():
    ...
    plt.savefig(f"/home/krzysztof400/Documents/code/university/ouap/{name}_plot.png")
