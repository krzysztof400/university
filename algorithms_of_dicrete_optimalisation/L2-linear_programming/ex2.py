...
def build_model(data):
    model = LpProblem(name="Which products to produce?", sense=LpMaximize)
    vars = {}
    prices = {}
    material_cost = {}
    ...
    for mname, minfo in data["machines"].items():
        model += (machine_hours[mname] <= minfo["availability_time"]), f"Machine_{mname}_Capacity"
...
if __name__ == "__main__":
    ...
    solve_and_save(prob, solver_name="CBC", lp_filename="linear_ex2.lp", solver_msg=True)
